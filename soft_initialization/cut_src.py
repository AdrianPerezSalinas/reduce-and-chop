...
class cut_model:
    ...
    def update_params1(self, params1):
        ...
        self.initial_state = self.circuit1.execute()    


    def update_t(self, t):
        self.t = t
        self.state_t = np.cos(np.pi / 2 * self.t) * self.zero_state + np.sin(np.pi / 2 * self.t)*self.initial_state

    
    def execute(self, reductor_params):
        self.reductor_params = reductor_params

        self.circuit_reductor.set_parameters(reductor_params)
        state = self.circuit_reductor.execute(initial_state=self.state_t.copy())
    ...
    def cost_reductor(self, reductor_params):
        self.reductor_params = reductor_params
        state, C, e = self.execute(reductor_params)
    ...
    def cost_input(self, params1):
        state, C, e = self.execute(self.reductor_params)
    ...
    def callback_cost_input(self, params1):
        if type(params1) != np.ndarray:
            params1 = params1.best.x
        state, C, e = self.execute(params1, self.reductor_params)

        try: 
            self.input_hist.append([C, e] )
        except:
            self.input_hist = []
            self.input_hist.append([C, e])
    ...
    def callback_cost_reductor_initialize(self, X):
        ...
        H, C, e = self.execute(self.reductor_params)
    ...
    def optimize_reductor(self, method = 'cma', options={'maxiter':150}, cb=None, sigma = None):
        ...
        if method == 'cma':
            ...
            es = fmin(lambda x: self.cost_reductor(x), self.reductor_params, sigma, callback = cb, options=options)
...
class cut_tfim(cut_model):
    ...
    def initialize_reductor(self, target_params, max_steps, method='cma', options={'maxiter':50}, options_end={'maxfevals':np.inf, 'maxiter':500}):
