...
counter = 0
folder = 'depth-limit2/results_soft_init_fixM/initialize_tfim'
...
#ax1.set_ylim([0, 1.1 * confident_entries / 2**qubits])
...
ax1.text(.5*lenM,confident_entries / 2**qubits * 1.3, r'$KL = $' + '%.4f'%(np.mean(KLs)) + r'$\pm$' + '%.4f'%(np.std(KLs)), 
