...
def main(qubits, random_layers, cb_layers, epsilon, seed):
    ...
    for res in rm.initialize_reductor1(target_params1, 250, method='cma', options={'maxfevals':np.inf, 'maxiter':50}):
    ...
    folder = '../results_param_init/initialize_tfim_double'
    ...
    if 'status' not in results1[-1].keys():
        ...
        for res in rm.initialize_reductor2(target_params2, 500, method='cma', options={'maxfevals':np.inf, 'maxiter':50}):
            res['params1'] = rm.params1
            res['params2'] = rm.params2
            res['params3'] = rm.params3
            results2.append(res)
